...
public class DatePicker extends WheelPicker {
        ...
        if (mode != MONTH_DAY) {
            ...
            yearView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
                ...
                public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                    ...
                    //需要根据年份及月份动态计算天数
                    days.clear();
                    int maxDays = DateUtils.calculateDaysInMonth(stringToYearMonthDay(item,0), stringToYearMonthDay(months.get(selectedMonthIndex),1));
        ...
        monthView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            ...
            public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                ...
                if (mode != YEAR_MONTH) {
                    ...
                    days.clear();
                    int maxDays = DateUtils.calculateDaysInMonth(stringToYearMonthDay(years.get(selectedYearIndex),0), stringToYearMonthDay(item,1));
                    for (int i = 1; i <= maxDays; i++) {
                        days.add(DateUtils.fillZero(i));
                    }
                    if (selectedDayIndex >= maxDays) {
    ...
    private int stringToYearMonthDay(String text,int state) {
        if(text.equals("")){
            if(state==0){
                text =years.get(0);
            }else{
